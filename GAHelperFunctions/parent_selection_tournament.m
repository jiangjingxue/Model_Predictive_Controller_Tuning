...
% tounament size tuning: 20 percent of the population size. (15 * 0.2 = 3)
% increase the torunment size will increase the selection pressure
...
for i = 1:pool_size
    ...
    % deal with duplicate parents ( duplicate parents are not allowed)
    if i > 1
        while selected_index == parents_index(i-1)
    ...
    % if there are mutiple max,always pick the first one
